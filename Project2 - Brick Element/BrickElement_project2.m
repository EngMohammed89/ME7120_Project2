...
% Eta=0;
% Zeta=0;
% J_m=Jacobian(Xi_I,Eta_I,Zeta_I,Xi,Eta,Zeta,x,y,z);
...
ke=zeros(24,24); 
 ...
 [bgpts,bgpw]=gauss([numbrickgauss,numbrickgauss,numbrickgauss]); % finds the gauss points and weights
  for i=1:size(bgpts,1)
      [Jac,dNdx,dNdy,dNdz]=Jacobian(Xi_I,Eta_I,Zeta_I,bgpts(i,1),bgpts(i,2),bgpts(i,3),x,y,z); % Finds J for the current Gauss point
      B=B_matrix(dNdx,dNdy,dNdz); % Finds B matrix for the current Gauss point
      ke=ke+bgpw(i)*B'*E*B*det(Jac); % Gauss integration part
  end
...
%   end
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   me=zeros(24,24);
   numbrickgauss=numbrickgauss+2; % Adds more gauss points for the mass matrix
  [bgpts,bgpw]=gauss([numbrickgauss,numbrickgauss,numbrickgauss]); % finds the gauss points and weights
  for i=1:size(bgpts,1) 
      Jac=Jacobian(bgpts(i,1),bgpts(i,2),bgpts(i,3),x,y,z); % Finds J for the current Gauss point
      N=Shape_function(Xi_I,Eta_I,Zeta_I,bgpts(i,1),bgpts(i,2),bgpts(i,3)); % Finds N for the current Gauss point
      me=me+bgpw(i)*N'*rho*N*det(Jac); % Gauss integration part
  end
...
%   
  ...
  surfs=[surfs;node5 node6 node7 node8 panelcolor];
  surfs=[surfs;node1 node4 node8 node5 panelcolor];
  surfs=[surfs;node1 node2 node6 node5 panelcolor];
  surfs=[surfs;node2 node3 node7 node6 panelcolor];
  surfs=[surfs;node3 node4 node8 node7 panelcolor];
